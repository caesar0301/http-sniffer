 ...
 * Static functions      *
 ***********************/
static void flow_reset(flow_t *f);
static int hook_packet(flow_t *f, packet_t *packet, BOOL src);
static int cal_packet(flow_t *f, packet_t *packet, BOOL src);
...
int FlowHTTPAdd(flow_t *f, http_pair_t *h)
{
	if(f->http_cnt == 0){
		f->http_f = h;
	}else{
...
int FlowPacketAdd(flow_t *f, packet_t *packet, u_int8_t src)
{
